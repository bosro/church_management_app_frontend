...
export class AttendanceService {
  ...
  createAttendanceEvent(eventData: Partial<AttendanceEvent>): Observable<AttendanceEvent> {
    const churchId = this.authService.getChurchId();
  ...
  updateAttendanceEvent(eventId: string, eventData: Partial<AttendanceEvent>): Observable<AttendanceEvent> {
    return from(
      this.supabase.update<AttendanceEvent>('attendance_events', eventId, eventData)
    ).pipe(
      map(({ data, error }) => {
        if (error) throw error;
        return data![0];
  ...
  deleteAttendanceEvent(eventId: string): Observable<void> {
    return from(
      this.supabase.delete('attendance_events', eventId)
    ).pipe(
      map(({ error }) => {
        if (error) throw error;
      })
  ...
  checkInVisitor(
    ...
    return from(
      (async () => {
        ...
        // Create attendance record
        const { data: attendanceData, error } = await this.supabase.insert<AttendanceRecord>(
          'attendance_records',
          {
            attendance_event_id: eventId,
  ...
  bulkCheckIn(eventId: string, memberIds: string[]): Observable<{ success: number, errors: string[] }> {
    return from(
      (async () => {
        let success = 0;
        const errors: string[] = [];
  ...
  // Get visitors
  getVisitors(page: number = 1, pageSize: number = 20): Observable<{ data: Visitor[], count: number }> {
    const churchId = this.authService.getChurchId();
    const offset = (page - 1) * pageSize;
  ...
  exportAttendanceReport(eventId: string): Observable<Blob> {
    return this.getAttendanceRecords(eventId).pipe(
      map((records) => {
        const headers = [
          'Name', 'Type', 'Member Number', 'Check-in Time', 'Check-in Method'
        ];
