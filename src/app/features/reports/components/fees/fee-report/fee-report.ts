...
export class FeeReport implements OnInit, OnDestroy {
  ...
  activeCardFilter: 'all' | 'paid' | 'outstanding' = 'all';
  ...
  loadReport(): void {
    ...
    this.errorMessage = '';
    this.schoolService
      .getAllStudentFees(this.selectedYear, this.selectedTerm)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (fees) => {
  ...
  setCardFilter(filter: 'all' | 'paid' | 'outstanding'): void {
    this.activeCardFilter = this.activeCardFilter === filter ? 'all' : filter; // toggle off if same
  }
  ...
  get studentSummaries(): any[] {
    ...
    // Apply card filter
    if (this.activeCardFilter === 'paid') {
      list = list.filter((s) => s.totalPaid > 0);
    } else if (this.activeCardFilter === 'outstanding') {
      list = list.filter((s) => s.totalBalance > 0);
    }

    return list.sort((a, b) => {
      const order: Record<string, number> = { unpaid: 0, partial: 1, paid: 2 };
      const statusDiff = (order[a.status] ?? 0) - (order[b.status] ?? 0);
      return statusDiff !== 0 ? statusDiff : b.totalBalance - a.totalBalance;
    });
  }

  get allStudentSummaries(): any[] {
    const map: { [key: string]: any } = {};
    this.outstandingFees.forEach((fee) => {
      const sid = fee.student_id;
      if (!map[sid]) {
        map[sid] = { totalDue: 0, totalPaid: 0, totalBalance: 0 };
      }
      map[sid].totalDue += Number(fee.amount_due);
      map[sid].totalPaid += Number(fee.amount_paid);
      map[sid].totalBalance += Number(fee.amount_due) - Number(fee.amount_paid);
  ...
  get grandTotalDue(): number {
    return this.allStudentSummaries.reduce((s, r) => s + r.totalDue, 0);
  }
  get grandTotalPaid(): number {
    return this.allStudentSummaries.reduce((s, r) => s + r.totalPaid, 0);
  }
  get grandTotalBalance(): number {
    return this.allStudentSummaries.reduce((s, r) => s + r.totalBalance, 0);
  }
  get studentsWithBalance(): number {
    return this.allStudentSummaries.filter((s) => s.totalBalance > 0).length;
  }
  ...
  private exportXLSX(fileName: string): void {
    ...
    XLSX.utils.book_append_sheet(
      ...
      XLSX.utils.json_to_sheet([
        ...
        { Info: 'Year', Value: this.selectedYear },
        ...
        {
          Info: 'Grand Balance (GHS)',
          Value: this.grandTotalBalance.toFixed(2),
        },
  ...
  private async exportPDFList(fileName: string) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(
      `Fee Outstanding Report — ${this.selectedTerm} ${this.selectedYear}`,
      pw / 2,
      14,
      { align: 'center' },
    );
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          ...
          '',
          '',
          this.formatCurrencyPDF(this.grandTotalDue),
          this.formatCurrencyPDF(this.grandTotalPaid),
          this.formatCurrencyPDF(this.grandTotalBalance),
        ],
      ],
