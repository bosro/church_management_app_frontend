...
export class FeeReport implements OnInit, OnDestroy {
  ...
  private exportSinglePDF(summary: any, fileName: string): void {
    const doc = new jsPDF({
    ...
    autoTable(doc, {
      ...
      body: summary.fees.map((f: any) => [
        f.fee_name,
        this.formatCurrency(f.amount_due),
        this.formatCurrency(f.amount_paid),
        this.formatCurrency(f.amount_due - f.amount_paid),
  ...
  private exportPDFList(fileName: string): void {
    const summaries = this.studentSummaries;
    const doc = new jsPDF({
    ...
    const stats = [
      ...
      `Grand Due: ${this.formatCurrency(this.grandTotalDue)}`,
      `Grand Paid: ${this.formatCurrency(this.grandTotalPaid)}`,
      `Outstanding: ${this.formatCurrency(this.grandTotalBalance)}`,
    ];
    ...
    autoTable(doc, {
      ...
      body: [
        ...summaries.map((s, i) => [
          ...
          this.formatCurrency(s.totalDue),
          this.formatCurrency(s.totalPaid),
          this.formatCurrency(s.totalBalance),
        ]),
        // Grand total row
        [
          '',
          ...
          '',
          '',
          this.formatCurrency(this.grandTotalDue),
          this.formatCurrency(this.grandTotalPaid),
          this.formatCurrency(this.grandTotalBalance),
        ],
      ],
